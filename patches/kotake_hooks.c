...
void EnTrt_GiveRedPotionForKoume(EnTrt* this, PlayState* play) {
    ...
    } else if (recomp_location_is_checked(GI_POTION_RED_BOTTLE)) {
...
void EnTrt_StartRedPotionConversation(EnTrt* this, PlayState* play) {
    if ((Message_GetState(&play->msgCtx) == TEXT_STATE_5) && Message_ShouldAdvance(play)) {
        if (this->textId == 0x88F) {
            ...
            if (!recomp_location_is_checked(GI_POTION_RED_BOTTLE) || !CHECK_WEEKEVENTREG(WEEKEVENTREG_RECEIVED_KOTAKE_BOTTLE)) {
    ...
    if (talkState == TEXT_STATE_DONE) {
        if (Message_ShouldAdvance(play)) {
            if ((Inventory_HasEmptyBottle() && !CHECK_WEEKEVENTREG(WEEKEVENTREG_RECEIVED_RED_POTION_FOR_KOUME)) ||
                ...
                !recomp_location_is_checked(GI_POTION_RED_BOTTLE)) {
                this->unk_3B2 = 12;
            } else {
