...
void KaleidoScope_UpdateItemCursor(PlayState* play) {
    ...
    s16 pad2;
    u8* sPlayerFormItems_reloc = (u8*) KaleidoManager_GetRamAddr(&sPlayerFormItems);
    s16* sEquipState_reloc = (s16*) KaleidoManager_GetRamAddr(&sEquipState);
    s16* sEquipMagicArrowSlotHoldTimer_reloc = (s16*) KaleidoManager_GetRamAddr(&sEquipMagicArrowSlotHoldTimer);
    s16* sEquipAnimTimer_reloc = (s16*) KaleidoManager_GetRamAddr(&sEquipAnimTimer);
                ...
                if ((pauseCtx->debugEditor == DEBUG_EDITOR_NONE) && !pauseCtx->itemDescriptionOn &&
                    ...
                    if (GET_PLAYER_FORM != PLAYER_FORM_HUMAN) {
                        ...
                        if (CHECK_BTN_ALL(CONTROLLER1(&play->state)->press.button, BTN_CLEFT)) {
                            if (sPlayerFormItems_reloc[GET_PLAYER_FORM] == BUTTON_ITEM_EQUIP(0, EQUIP_SLOT_C_LEFT)) {
                                Audio_PlaySfx(NA_SE_SY_ERROR);
                                return;
                            }
                        } else if (CHECK_BTN_ALL(CONTROLLER1(&play->state)->press.button, BTN_CDOWN)) {
                            if (sPlayerFormItems_reloc[GET_PLAYER_FORM] == BUTTON_ITEM_EQUIP(0, EQUIP_SLOT_C_DOWN)) {
                                Audio_PlaySfx(NA_SE_SY_ERROR);
                                return;
                            }
                        } else if (CHECK_BTN_ALL(CONTROLLER1(&play->state)->press.button, BTN_CRIGHT)) {
                            if (sPlayerFormItems_reloc[GET_PLAYER_FORM] == BUTTON_ITEM_EQUIP(0, EQUIP_SLOT_C_RIGHT)) {
                                Audio_PlaySfx(NA_SE_SY_ERROR);
                                return;
                            }
                        }
                    ...
                    *sEquipMagicArrowSlotHoldTimer_reloc = 0;
                    *sEquipState_reloc = EQUIP_STATE_MOVE_TO_C_BTN;
                    *sEquipAnimTimer_reloc = 10;
                    ...
                    if ((pauseCtx->equipTargetItem == ITEM_ARROW_FIRE) ||
                        ...
                        pauseCtx->equipAnimAlpha = *sEquipState_reloc = 0; // EQUIP_STATE_MAGIC_ARROW_GROW_ORB
                        *sEquipAnimTimer_reloc = 6;
                    } else {
