...
s32 func_808B849C(DoorWarp1* this, PlayState* play) {
    ...
    if ((play->sceneId == SCENE_MITURIN_BS) && !recomp_location_is_checked(LOCATION_REMAINS_ODOLWA)) {
        ret = 1;
    //} else if ((play->sceneId == SCENE_HAKULOCATIONN_BS) && !CHECK_QUEST_ITEM(QUEST_REMAINS_GOHT)) {
    } else if ((play->sceneId == SCENE_HAKUGIN_BS) && !recomp_location_is_checked(LOCATION_REMAINS_GOHT)) {
        ret = 2;
    //} else if ((play->sceneId == SCENE_SEA_BS) && !CHECK_QUEST_ITEM(QUEST_REMAINS_GYORG)) {
    } else if ((play->sceneId == SCENE_SEA_BS) && !recomp_location_is_checked(LOCATION_REMAINS_GYORG)) {
        ret = 3;
    //} else if ((play->sceneId == SCENE_INISIE_BS) && !CHECK_QUEST_ITEM(QUEST_REMAINS_TWINMOLD)) {
    } else if ((play->sceneId == SCENE_INISIE_BS) && !recomp_location_is_checked(LOCATION_REMAINS_TWINMOLD)) {
        ret = 4;
    }
    return ret;
}
